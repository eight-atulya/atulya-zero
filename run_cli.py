...
context: AtulyaContext = None # type: ignore
input_lock = threading.Lock()


# Main conversation loop
async def chat(context: AtulyaContext):
    
    # start the conversation loop  
    while True:
        # ask user for message
        with input_lock:
            timeout = context.atulya0.get_data("timeout") # how long the atulya is willing to wait
            if not timeout: # if atulya wants to wait for user input forever
                ...
                if not user_input:
                    user_input = context.atulya0.read_prompt("fw.msg_timeout.md")
        ...
        # send message to atulya0, 
        assistant_response = await context.communicate(UserMessage(user_input, [])).result()
        
        # print atulya0 response
        PrintStyle(font_color="white",background_color="#1D8348", bold=True, padding=True).print(f"{context.atulya0.atulya_name}: reponse:")        
        PrintStyle(font_color="white").print(f"{assistant_response}")        
                        

# User intervention during atulya streaming
def intervention():
    if context.streaming_atulya and not context.paused:
        context.paused = True # stop atulya streaming
        ...
        if user_input: context.streaming_atulya.intervention = UserMessage(user_input, []) # set intervention message if non-empty
        context.paused = False # continue atulya streaming 
...
def run():
    ...
    # initialize context
    config = initialize()
    context = AtulyaContext(config)

    # Start the key capture thread for user intervention during atulya streaming
