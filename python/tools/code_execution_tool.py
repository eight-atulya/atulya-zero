...
class CodeExecution(Tool):
    ...
    async def execute(self, **kwargs):
        ...
        await self.atulya.handle_intervention()  # wait for intervention and handle it, if paused
        ...
        if runtime == "python":
            ...
            response = self.atulya.read_prompt(
                "fw.code.runtime_wrong.md", runtime=runtime
            )

        if not response:
            response = self.atulya.read_prompt(
                "fw.code.info.md", info=self.atulya.read_prompt("fw.code.no_output.md")
            )
        return Response(message=response, break_loop=False)

    def get_log_object(self):
        return self.atulya.context.log.log(
            type="code_exe",
    ...
    async def prepare_state(self, reset=False, session=None):
        self.state = self.atulya.get_data("_cet_state")
        if not self.state or reset:

            # initialize docker container if execution in docker is configured
            if not self.state and self.atulya.config.code_exec_docker_enabled:
                docker = DockerContainerManager(
                    logger=self.atulya.context.log,
                    name=self.atulya.config.code_exec_docker_name,
                    image=self.atulya.config.code_exec_docker_image,
                    ports=self.atulya.config.code_exec_docker_ports,
                    volumes=self.atulya.config.code_exec_docker_volumes,
                )
            ...
            if 0 not in shells:
                if self.atulya.config.code_exec_ssh_enabled:
                    pswd = (
                        self.atulya.config.code_exec_ssh_pass
                        if self.atulya.config.code_exec_ssh_pass
                        else await rfc_exchange.get_root_password()
                    )
                    shell = SSHInteractiveSession(
                        self.atulya.context.log,
                        self.atulya.config.code_exec_ssh_addr,
                        self.atulya.config.code_exec_ssh_port,
                        self.atulya.config.code_exec_ssh_user,
                        pswd,
                    )
                else:
        ...
        self.atulya.set_data("_cet_state", self.state)
    ...
    async def terminal_session(self, session: int, command: str, reset: bool = False):
        ...
        for i in range(2):
            try:
                ...
                ).print(f"{self.atulya.atulya_name} code execution output")
    ...
    async def get_terminal_output(
        ...
        while True:
            ...
            await self.atulya.handle_intervention()
            ...
            if partial_output:
                ...
                truncated_output = truncate_text(
                    atulya=self.atulya, output=full_output, threshold=10000
                )
            ...
            if now - start_time > max_exec_timeout:
                sysinfo = self.atulya.read_prompt(
                    "fw.code.max_time.md", timeout=max_exec_timeout
                )
                response = self.atulya.read_prompt("fw.code.info.md", info=sysinfo)
                if truncated_output:
    ...
    async def reset_terminal(self, session=0, reason: str | None = None):
        ...
        response = self.atulya.read_prompt(
            "fw.code.info.md", info=self.atulya.read_prompt("fw.code.reset.md")
        )
        self.log.update(content=response)
        return response
