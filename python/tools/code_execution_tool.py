...
class CodeExecution(Tool):
    ...
    async def execute(self, **kwargs):
        ...
        if not response:
            response = self.atulya.read_prompt("fw.code_no_output.md")
    ...
    #     ).print(f"{self.atulya.atulya_name}: Using tool '{self.name}'")
    #     self.log = self.atulya.context.log.log(
    #         type="code_exe",
    ...
    async def prepare_state(self, reset=False):
        self.state = self.atulya.get_data("_cot_state")
        if not self.state or reset:

            # initialize docker container if execution in docker is configured
            if self.atulya.config.code_exec_docker_enabled:
            ...
            await shell.connect()
        self.atulya.set_data("_cot_state", self.state)
    ...
    async def reset_terminal(self, session=0):
        ...
        await self.prepare_state(reset=True)
        response = self.atulya.read_prompt("fw.code_reset.md")
