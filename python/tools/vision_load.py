...
class VisionLoad(Tool):
            ...
            if path not in self.images_dict:
                ...
                if mime_type and mime_type.startswith("image/"):
                    try:
                        ...
                        self.atulya.context.log.log("warning", f"Error processing image {path}: {e}")
    ...
    async def after_execution(self, response: Response, **kwargs):
        ...
        if self.images_dict:
            ...
            self.atulya.hist_add_message(
                False, content=msg, tokens=TOKENS_ESTIMATE * len(content)
            )
        else:
            self.atulya.hist_add_tool_result(self.name, "No images processed")
        ...
        ).print(f"{self.atulya.atulya_name}: Response from tool '{self.name}'")
