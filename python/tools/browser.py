...
class Browser(Tool):
    ...
    async def prepare_state(self, reset=False):
        self.state = self.atulya.get_data("_browser_state")
        if not self.state or reset:
            self.state = State(browser=BrowserManager())
        self.atulya.set_data("_browser_state", self.state)
    ...
    def cleanup_history(self):
        ...
        for msg in self.atulya.history.current.messages:
            cleanup_message(msg)
        
        for prev in self.atulya.history.topics:
            if not prev.summary:
