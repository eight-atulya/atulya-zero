...
class State:
    @staticmethod
    async def create(atulya: Atulya):
        state = State(atulya)
        return state

    def __init__(self, atulya: Atulya):
        self.atulya = atulya
        self.context = None
        self.task = None
        self.use_atulya = None
        self.browser = None
        self.iter_no = 0
    ...
    def start_task(self, task: str):
        ...
        if not self.task:
            self.task = defer.DeferredTask(
                thread_name="BrowserAtulya" + self.atulya.context.id
            )
            if self.atulya.context.task:
                self.atulya.context.task.add_child_task(self.task, terminate_thread=True)
    ...
    async def _run_task(self, task: str):
        ...
        model = models.get_model(
            ...
            provider=self.atulya.config.browser_model.provider,
            name=self.atulya.config.browser_model.name,
        ...
        self.use_atulya = browser_use.Atulya(
            ...
            llm=model,
            use_vision=self.atulya.config.browser_model.vision,
            system_prompt_class=CustomSystemPrompt,
            controller=controller,
        )

        self.iter_no = get_iter_no(self.atulya)

        # orig_err_hnd = self.use_atulya._handle_step_error
        # def new_err_hnd(*args, **kwargs):
        #     if isinstance(args[0], InterventionException):
        #         raise args[0]
        #     return orig_err_hnd(*args, **kwargs)
        # self.use_atulya._handle_step_error = new_err_hnd

        result = await self.use_atulya.run()
        return result
    ...
    def override_hooks(self):
        ...
        def override_hook(func):
            async def wrapper(*args, **kwargs):
                await self.atulya.wait_if_paused()
                if self.iter_no != get_iter_no(self.atulya):
    ...
    async def get_page(self):
        if self.use_atulya:
            return await self.use_atulya.browser_context.get_current_page()
...
class BrowserAtulya(Tool):
    ...
    #     await self.atulya.hist_add_tool_result(self.name, response.message)
            ...
            try:
                ...
                async def _get_update():
                    ...
                    path = files.get_abs_path(
                        persist_chat.get_chat_folder_path(atulya.context.id),
                        "browser",
                        "screenshots",
                        f"{self.guid}.png",
                    )
    ...
    async def prepare_state(self, reset=False):
        self.state = self.atulya.get_data("_browser_agent_state")
        if not self.state or reset:
            self.state = await State.create(self.atulya)
        self.atulya.set_data("_browser_agent_state", self.state)
    ...
    def update_progress(self, text):
        ...
        self.atulya.context.log.set_progress(progress)
