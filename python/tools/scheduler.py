...
class SchedulerTool(Tool):
    ...
    async def run_task(self, **kwargs) -> Response:
        ...
        if task.context_id == self.atulya.context.id:
    ...
    async def delete_task(self, **kwargs) -> Response:
        ...
        context = None
        if task.context_id:
            context = AtulyaContext.get(task.context_id)
        ...
        if context and context.id == task.uuid:
            AtulyaContext.remove(context.id)
    ...
    async def create_scheduled_task(self, **kwargs) -> Response:
        ...
        # Validate cron expression, atulya might hallucinate
        ...
        task = ScheduledTask.create(
            ...
            context_id=None if dedicated_context else self.atulya.context.id
        )
