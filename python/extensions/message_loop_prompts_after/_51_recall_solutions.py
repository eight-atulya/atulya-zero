...
class RecallSolutions(Extension):
    ...
    async def search_solutions(self, loop_data: LoopData, **kwargs):
        ...
        # msgs_text = self.atulya.concat_messages(
        #     self.atulya.history[-RecallSolutions.HISTORY :]
        # )  # only last X messages
        # msgs_text = self.atulya.history.current.output_text()
        msgs_text = self.atulya.history.output_text()[-RecallSolutions.HISTORY:]

        system = self.atulya.read_prompt(
        ...
        query = await self.atulya.call_utility_model(
        ...
        db = await Memory.get(self.atulya)
        ...
        if instruments:
            ...
            instruments_prompt = self.atulya.read_prompt(
                "atulya.system.instruments.md", instruments=instruments_text
            )
        ...
        if solutions:
            ...
            solutions_prompt = self.atulya.parse_prompt(
                "atulya.system.solutions.md", solutions=solutions_text
            )
