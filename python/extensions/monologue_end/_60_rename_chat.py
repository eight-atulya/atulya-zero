...
class RenameChat(Extension):
    ...
    async def change_name(self):
        try:
            # prepare history
            history_text = self.atulya.history.output_text()
            ctx_length = int(self.atulya.config.utility_model.ctx_length * 0.7)
            history_text = tokens.trim_to_tokens(history_text, ctx_length, "start")
            # prepare system and user prompt
            system = self.atulya.read_prompt("fw.rename_chat.sys.md")
            current_name = self.atulya.context.name
            message = self.atulya.read_prompt(
                "fw.rename_chat.msg.md", current_name=current_name, history=history_text
            )
            # call utility model
            new_name = await self.atulya.call_utility_model(
            ...
            if new_name:
                ...
                self.atulya.context.name = new_name
