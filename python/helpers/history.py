...
class Topic(Record):
    ...
    async def compress_attention(self) -> bool:
        ...
        if len(self.messages) > 2:
            ...
            sum_msg_content = self.history.atulya.parse_prompt(
    ...
    async def summarize_messages(self, messages: list[Message]):
        ...
        summary = await self.history.atulya.call_utility_model(
            system=self.history.atulya.read_prompt("fw.topic_summary.sys.md"),
            message=self.history.atulya.read_prompt(
...
class Bulk(Record):
    ...
    async def summarize(self):
        self.summary = await self.history.atulya.call_utility_model(
            system=self.history.atulya.read_prompt("fw.topic_summary.sys.md"),
            message=self.history.atulya.read_prompt(
...
def deserialize_history(json_data: str, atulya) -> History:
    history = History(atulya=atulya)
    if json_data:
