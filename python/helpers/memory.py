...
class Memory:
    ...
    @staticmethod
    async def get(atulya: Atulya):
        memory_subdir = atulya.config.memory_subdir or "default"
        if Memory.index.get(memory_subdir) is None:
            log_item = atulya.context.log.log(
                type="util",
            ...
            db = Memory.initialize(
                ...
                models.get_model(
                    ...
                    atulya.config.embeddings_model.provider,
                    atulya.config.embeddings_model.name,
                    **atulya.config.embeddings_model.kwargs,
                ),
                memory_subdir,
                False,
            )
            Memory.index[memory_subdir] = db
            wrap = Memory(atulya, db, memory_subdir=memory_subdir)
            if atulya.config.knowledge_subdirs:
                await wrap.preload_knowledge(
                    log_item, atulya.config.knowledge_subdirs, memory_subdir
                )
            return wrap
        else:
            return Memory(
    ...
    async def search_similarity_threshold(
        ...
        #rate limiter
        await self.atulya.rate_limiter(
            model_config=self.atulya.config.embeddings_model, input=query)
    ...
    async def insert_documents(self, docs: list[Document]):
        ...
        if ids:
            ...
            await self.atulya.rate_limiter(
                model_config=self.atulya.config.embeddings_model, input=docs_txt)
...
def get_memory_subdir_abs(atulya: Atulya) -> str:
    return files.get_abs_path("memory", atulya.config.memory_subdir or "default")


def get_custom_knowledge_subdir_abs(atulya: Atulya) -> str:
    for dir in atulya.config.knowledge_subdirs:
        if dir != "default":
