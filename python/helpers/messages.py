...
def truncate_text(atulya, output, threshold=1000):
    threshold = int(threshold)
    if not threshold or len(output) <= threshold:
        return output

    # Adjust the file path as needed
    placeholder = atulya.read_prompt(
...
def truncate_dict_by_ratio(atulya, data: dict|list|str, threshold_chars: int, truncate_to: int):
    ...
    def process_item(item):
        if isinstance(item, dict):
            ...
            for key, value in item.items():
                ...
                if cumulative_size + size > threshold_chars:
                    truncated_dict[key] = truncate_text(
                        atulya, serialized_value, truncate_to
                    )
                else:
            ...
            for value in item:
                ...
                if cumulative_size + size > threshold_chars:
                    truncated_list.append(
                        truncate_text(atulya, serialized_value, truncate_to)
                    )
                else:
                    cumulative_size += size
            ...
            if len(item) > threshold_chars:
                return truncate_text(atulya, item, truncate_to)
            return item
