...
from atulya import Atulya, AtulyaContext, UserMessage
...
class TaskScheduler:
    ...
    async def __new_context(self, task: Union[ScheduledTask, AdHocTask, PlannedTask]) -> AtulyaContext:
        if not task.context_id:
            raise ValueError(f"Task {task.name} has no context ID")

        config = initialize()
        context: AtulyaContext = AtulyaContext(config, id=task.context_id, name=task.name)
    ...
    async def _get_chat_context(self, task: Union[ScheduledTask, AdHocTask, PlannedTask]) -> AtulyaContext:
        context = AtulyaContext.get(task.context_id) if task.context_id else None

        if context:
            assert isinstance(context, AtulyaContext)
            self._printer.print(
    ...
    async def _persist_chat(self, task: Union[ScheduledTask, AdHocTask, PlannedTask], context: AtulyaContext):
    ...
    async def _run_task(self, task: Union[ScheduledTask, AdHocTask, PlannedTask], task_context: str | None = None):
        ...
        async def _run_task_wrapper(task_uuid: str, task_context: str | None = None):
            ...
            # the atulya instance - init in try block
            atulya = None
            ...
            try:
                ...
                # Ensure the context is properly registered in the AtulyaContext._contexts
                # This is critical for the polling mechanism to find and stream logs
                # Dict operations are atomic
                # AtulyaContext._contexts[context.id] = context
