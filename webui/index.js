...
window.pauseAtulya = async function (paused) {
    try {
        const resp = await sendJsonData("/pause", { paused: paused, context });
    } catch (e) {
        window.toastFetchError("Error pausing atulya", e)
    }
}
...
window.nudge = async function () {
    try {
        ...
        toastFetchError("Error nudging atulya", e)
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    ['dratulyaer', 'dragover', 'dragleave', 'drop'].forEach(eventName => {
    ...
    // Handle drag enter
    document.addEventListener('dratulyaer', (e) => {
        dragCounter++;
        if (dragCounter === 1) {
