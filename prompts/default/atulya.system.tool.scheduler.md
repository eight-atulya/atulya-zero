...
The task scheduler is a part of atulya-zero enabling the system to execute
...
Be careful not to create recursive prompt, do not send a message that would make the atulya schedule more tasks, no need to mention the interval in message, just the objective.
...
#### AdHoc - type="adhoc"
This type of task is run manually and does not follow any schedule. It can be run explicitly by "scheduler:run_task" atulya tool or by the user in the UI.
