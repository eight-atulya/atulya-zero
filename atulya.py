...
class AtulyaContext:
    ...
    _contexts: dict[str, "AtulyaContext"] = {}
    _counter: int = 0
    ...
    def __init__(
        ...
        self.atulya0 = atulya0 or Atulya(0, self.config, self)
        self.paused = paused
        ...
        if existing:
            AtulyaContext.remove(self.id)
        self._contexts[self.id] = self

    @staticmethod
    def get(id: str):
        return AtulyaContext._contexts.get(id, None)

    @staticmethod
    def first():
        if not AtulyaContext._contexts:
            return None
        return list(AtulyaContext._contexts.values())[0]

    @staticmethod
    def remove(id: str):
        context = AtulyaContext._contexts.pop(id, None)
    ...
    def reset(self):
        self.kill_process()
        self.log.reset()
        self.atulya0 = Atulya(0, self.config, self)
        self.streaming_atulya = None
        self.paused = False

    def nudge(self):
        self.kill_process()
        self.paused = False
        if self.streaming_atulya:
            current_atulya = self.streaming_atulya
        else:
            current_atulya = self.atulya0

        self.task = self.run_task(current_atulya.monologue)
        return self.task
    ...
    def communicate(self, msg: "UserMessage", broadcast_level: int = 1):
        ...
        if self.task and self.task.is_alive():
            # set intervention messages to atulya(s):
            intervention_atulya = current_atulya
            while intervention_atulya and broadcast_level != 0:
                intervention_atulya.intervention = msg
                broadcast_level -= 1
                intervention_atulya = intervention_atulya.data.get(
                    Atulya.DATA_NAME_SUPERIOR, None
                )
        else:
            self.task = self.run_task(self._process_chain, current_atulya, msg)
    ...
    # this wrapper ensures that superior atulyas are called back if the chat was loaded from file and original callstack is gone
    async def _process_chain(self, atulya: "Atulya", msg: "UserMessage|str", user=True):
        try:
            msg_template = (
                atulya.hist_add_user_message(msg)  # type: ignore
                if user
                else atulya.hist_add_tool_result(
...
class Atulya:
    ...
    def __init__(
        self, number: int, config: AtulyaConfig, context: AtulyaContext | None = None
    ):
        ...
        # atulya context
        self.context = context or AtulyaContext(config)

        # non-config vars
        self.number = number
        self.atulya_name = f"Atulya {self.number}"
    ...
    async def monologue(self):
        while True:
            try:
                ...
                # let the atulya run message loop until he stops it with a response tool
                while True:
                    ...
                    try:
                        ...
                        # output that the atulya is starting
                        PrintStyle(
                            bold=True,
                            font_color="green",
                            padding=True,
                            background_color="white",
                        ).print(f"{self.atulya_name}: Generating")
                        log = self.context.log.log(
                            type="atulya", heading=f"{self.atulya_name}: Generating"
                        )

                        async def stream_callback(chunk: str, full: str):
                            # output the atulya response stream
                            if chunk:
                        ...
                        if (
                            self.loop_data.last_response == atulya_response
                        ):  # if assistant_response is the same as last message in history, let him know
                            # Append the assistant's response to the history
                            self.hist_add_ai_response(atulya_response)
                            ...
                            # Append the assistant's response to the history
                            self.hist_add_ai_response(atulya_response)
                            # process tools requested in atulya message
                            tools_result = await self.process_tools(atulya_response)
            ...
            # exceptions outside message loop:
                ...
                self.context.streaming_atulya = None  # unset current streamer
    ...
    def parse_prompt(self, file: str, **kwargs):
        ...
        ):  # if atulya has custom folder, use it and use default as backup
    ...
    async def process_tools(self, msg: str):
        # search for tool usage requests in atulya message
        ...
        if tool_request is not None:
            ...
            self.context.log.log(
                type="error", content=f"{self.atulya_name}: Message misformat"
            )
    ...
    async def call_extensions(self, folder: str, **kwargs) -> Any:
        ...
        for cls in classes:
            await cls(atulya=self).execute(**kwargs)
